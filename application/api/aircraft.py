...
@aircraft_router.post(
    "/aircraft/", response_model=schemas.Aircraft, status_code=201, tags=["aircraft"]
)
async def create_aircraft(
    aircraft: schemas.Aircraft, session: Session = Depends(get_session)
):
    return AircraftCRUD.create_object(session, aircraft)


@aircraft_router.get(
    "/aircraft/{serial_number}", response_model=schemas.Aircraft, tags=["aircraft"]
)
async def read_aircraft(serial_number: str, session: Session = Depends(get_session)):
...
@aircraft_router.get(
    "/aircraft/", response_model=list[schemas.Aircraft], tags=["aircraft"]
)
...
@aircraft_router.delete("/aircraft/{serial_number}", status_code=204, tags=["aircraft"])
async def delete_aircraft(serial_number: str, session: Session = Depends(get_session)):
